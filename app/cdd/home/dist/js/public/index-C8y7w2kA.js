...
const toHandlerKey = cacheStringFunction(
  (str) => {
    const s = str ? "on".concat(capitalize(str)) : "";
    return s;
  }
);
...
const toDisplayString = (val) => {
  return isString(val) ? val : val == null ? "" : isArray(val) || isObject$2(val) && (val.toString === objectToString || !isFunction$1(val.toString)) ? isRef$1(val) ? toDisplayString(val.value) : JSON.stringify(val, replacer, 2) : String(val);
};
const replacer = (_key, val) => {
  if (isRef$1(val)) {
    ...
    return {
      ["Map(".concat(val.size, ")")]: [...val.entries()].reduce(
        (entries, [key, val2], i) => {
...
function createNamespace(name2) {
  const prefixedName = "van-".concat(name2);
  return [
    prefixedName,
...
const BORDER_SURROUND = "".concat(BORDER, "--surround");
function withInstall(options) {
  options.install = (app) => {
    const { name: name2 } = options;
    if (name2) {
      app.component(name2, options);
      app.component(camelize("-".concat(name2)), options);
    }
  };
  return options;
}
...
var stdin_default$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(props, {
    ...
    const getOffsetWithMinusString = (val) => val.startsWith("-") ? val.replace("-", "") : "-".concat(val);
...
var stdin_default$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(props, {
    ...
    const classPrefix = computed(() => props.classPrefix || (config == null ? void 0 : config.iconPrefix) || bem$2());
    return () => {
      const {
        tag,
        dot,
...
const _imports_0 = "/img/banner@2x-CytXxbee.png";
...
//# sourceMappingURL=index-C8y7w2kA.js.map
